 ...
 *
 * The panel is deliberately separate from Notes' own front-end bundle. It
 * appears on any front-end page a teacher visits, while the Notes workspace
 * has its own trigger and its own gate; keeping the assets apart means the two
 * surfaces cannot start depending on each other's load order.
 *
 * @package TBT_Notes
 */
...
class TBT_Notes_Progress_Panel {
	...
	protected function should_load() {
		 ...
		 *
		 * The panel follows a teacher around the site, which is right during a
		 * lesson and noise the rest of the time. A site that wants it only on
		 * lesson pages can say so here without touching the plugin.
		 *
		 * @param bool $load Whether to load.
		 */
		if ( ! apply_filters( 'tbt_notes_progress_should_load', true ) ) {
			return false;
		}
